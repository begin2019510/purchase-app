...
// 采购 Modal
// ============================================================
function openModal(){document.getElementById('editId').value='';document.getElementById('modalTitle').textContent='新增采购';document.getElementById('fName').value='';document.getElementById('fReason').value='';document.getElementById('fName').style.display='';document.getElementById('aiEvalResult').style.display='none';document.getElementById('aiEvalResult').textContent='';document.getElementById('chatArea').style.display='none';document.getElementById('chatMessages').innerHTML='';purchaseChatHistory=[];purchaseEvalContext='';document.getElementById('evalPhase').style.display='';document.getElementById('detailPhase').style.display='none';document.getElementById('editPhase').style.display='none';var instGroup=document.getElementById('installmentGroup');var instInput=document.getElementById('fInstallments');var instPreview=document.getElementById('installmentPreview');var instVal=i['分期期数']||0;if(instGroup)instGroup.style.display=instVal>0?'':'none';if(instInput){instInput.value=instVal||'';instInput.oninput=function(){var periods=parseInt(this.value)||0;var price=parseFloat(document.getElementById('fPriceEdit').value)||0;var qty=parseInt(document.getElementById('fQtyEdit').value)||1;var total=price*qty;if(instPreview)instPreview.textContent=periods>0?'每期 \u00a5'+(total/periods).toFixed(2):'';if(instGroup)instGroup.style.display=periods>0?'':'none';}}if(instPreview)instPreview.textContent=instVal>0?'每期 \u00a5'+(((i['单价']||0)*(i['数量']||1))/instVal).toFixed(2):'';document.getElementById('overlay').classList.add('active')}
