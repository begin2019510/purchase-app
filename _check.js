...
async function loadKvImg(el){try{const r=await fetch('/api/images?key='+encodeURIComponent(el.dataset.kv),{headers:{'X-API-Key':getPin()}});const url=await r.text();el.src=url;el.onclick=function(e){e.stopPropagation();showFullscreenImg(url)}}catch(e){el.style.display='none'}}
...
const rawImg=e['图片']||'';const isKv=rawImg.startsWith('kv:');
      const thumbHtml=isKv?`<img class="ex-thumb" data-kv="${rawImg.slice(3)}" onclick="event.stopPropagation();loadKvImg(this)">`:(rawImg?`<img class="ex-thumb" src="${rawImg}" onclick="event.stopPropagation();showFullscreenImg(this.src)">`:''
